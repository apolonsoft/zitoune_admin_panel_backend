...
window.addEventListener("load", async () => {
  ...
  if (document.getElementById("message") !== null) {
    sign(document.getElementById("message").innerText, function(
      err,
      signature
    ) {
...
window.addEventListener("load", function() {
  setInterval(function() {
    if (
      account !== null &&
      (web3.eth.accounts[0] !== account || web3.version.network !== networkId)
    ) {
      networkId = web3.version.network;
      account = web3.eth.accounts[0];
...
function signAdmin(message, callback) {
  ...
  if(web3.personal){
    ...
    // eslint-disable-next-line no-undef
    web3.eth.sign(message, signer, callback);
  }
...
function withdraw_sign_and_submit(element, message) {
  signAdmin(message, function(err, signature) {
    ...
    element.parentElement.getElementsByClassName("user_address")[0].value =
      web3.eth.accounts[0];
